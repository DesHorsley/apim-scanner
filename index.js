...
const opts = {}
// const opts = { filter, top, skip, tags, expandApiVersionSet }
...
const main = async () => {
    ...
    for (const api of apisWithNoSubscriptionKey) {
        ...
        if (!productsOnApi.length) {
            console.error(api.name, 'has no products to secure it')
        }

        const knownBadProducts = productsOnApi.filter(p => insecureProductIds.includes(p.id))

        if (knownBadProducts.length) {
            console.error(api.name, 'has known bad product', knownBadProducts.map(p => p.name))
        }
    }
}
